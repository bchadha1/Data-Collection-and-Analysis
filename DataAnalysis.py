...
def trainAndClassify(tweets, classifier, method, feature_set, fileprefix):
    ...
    if '1step' == method:
        for k in range(FOLDS):
            ...
            '######################'
            print
            classifier_tot.show_most_informative_features(NUM_SHOW_FEATURES)
            print
            '######################'
            ...
            'Confusion Matrix'
            print
            nltk.ConfusionMatrix(test_truth, test_predict)

            accuracies.append(accuracy_tot)
        print
        "Accuracies:", accuracies
        print
        "Average Accuracy:", sum(accuracies) / FOLDS
        ...
        accuracies = []
        for k in range(FOLDS):
            (v_train_obj, v_train_sen, v_test_obj, v_test_sen, test_truth) = getTrainingAndTestData(tweets, FOLDS, k,
                                                                                                    method, feature_set)
            ...
            '######################'
            print
            'Objectivity Classifier :', classifier
            ...
            '######################'
            print
            classifier_obj.show_most_informative_features(NUM_SHOW_FEATURES)
            print
            '######################'
            ...
            'Confusion Matrix'
            print
            nltk.ConfusionMatrix(test_truth_obj, test_predict_obj)

            print
            '######################'
            print
            'Sentiment Classifier :', classifier
            accuracy_sen = nltk.classify.accuracy(classifier_sen, v_test_sen)
            print
            'Accuracy :', accuracy_sen
            print
            '######################'
            print
            classifier_sen.show_most_informative_features(NUM_SHOW_FEATURES)
            print
            '######################'
            ...
            if (len(test_truth_sen) > 0):
                print
                nltk.ConfusionMatrix(test_truth_sen, test_predict_sen)
...
def main(argv):
    ...
    if (len(fileprefix) == 0 or len(classifierNames) == 0 or len(methodNames) == 0 or len(ngramVals) == 0 or len(
            negtnVals) == 0):
        print
        __usage__
        return
